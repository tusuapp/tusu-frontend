...
const typeArray: Object = {tutor: "student_online_status", student: "tutor_online_status"}

const StatusButton: React.FC<IButton> = ({id, scheduleInfo, status, type, bbb}) => {
  //  console.log(bbb)
                ...
                case "inprogress":
                    ...
                    if (whatleftDays > 1) {
                        ...
                        if (whatLeft.asMinutes() > 0) {
                            ...
                            } else if (/*scheduleInfo.student_online_status == "online" && */ scheduleInfo.tutor_online_status == "online") {
    ...
    const handleClassStart = (id: number) => {
        if (status === "completed" && bblink) {
         //   window.open(bblink, '_blank', 'noreferrer');
            ...
            router.push(`/${type}/classes/${id}`);
    };

    return (
        <button
            className={status === "completed" ? "button btn-success" : (whatLeft.asDays() > 0
                ? `button button--${xstatus}`
