...
const FilterDropdown: React.FC<IFilterDropdown> = ({onApplyFilter, parentHandleClear}) => {
        ...
        try {
            ...
            Object.keys(searchVals).forEach((key: any, index: any) => {
                ...
                if (key == "country" && val && val.value) {
                    let xx = `_where[${i++}][country.id]=${val.value}`
                    _where.push(xx)
                }
                if (key == "gender" && val && val.value) {
                    let xx = `_where[${i++}][tutor_details.gender]=${val.value}`
                    _where.push(xx)
                }
                if (key == "experienceYears" && val && val.value) {

                    let xx = `_where[${i++}][tutor_details.experience]=${val.value}`
                    _where.push(xx)

                }
                if (key == "disciplines" && val) {
                    _where.push(buildOrWhereQry("tutor_details.discipline_id.id_in", val, i++))
                }
                if (key == "subjects" && val) {
                    _where.push(buildOrWhereQry("tutor_details.subject_id.id_in", val, i++))
                }
                if (key == "knownLanguages" && val) {
                    _where.push(buildOrWhereQry("tutor_details.language.id_in", val, i++))
                }
                if (key == "hourlyCharges" && val && val[1] > 0) {
                    let xx = `_where[${i++}][tutor_details.hourly_charge_lte]=${val[1]}&_where[${i++}][tutor_details.hourly_charge_gte]=${val[0]}`
                    _where.push(xx)
                }
            })
        } catch (e) {
    ...
    function buildOrWhereQry(key: string, val: Array<{ value: any; }>, index: number) {
        ...
        if (val && val.length) {
            val.forEach((d: { value: any; }) => {
                let xx = `_where[${index}][${key}]=${d.value}`
                orWhere.push(xx)
            })
        }
        return orWhere.join('&')
