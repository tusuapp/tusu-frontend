...
const IncomingChat = ({ content, timestamp }: any) => {
  return (
...
const OutgoingChat = ({ content, timestamp }: any) => {
  return (
...
function Message() {
  ...
  useEffect(() => {
    if (user && user.uuid && isSocket) {
      socket.emit("user_chat_with_uuid", {
        userId: user.uuid,
        uuid: room_id,
        perPage: 50,
        currentPage: 1,
      ...
      socket.on("user_chat_with_uuid_" + user.uuid, ({ tab, result }) => {
        ...
        // msg = _.orderBy(result, "updated_at", "asc");
        ...
        <div className="flex-grow-1">
          ...
          <section className="msger">
            <main className="msger-chat">
              {oldMsgData.data?.map((message, i) => {
                if (message.from_id === user.uuid) {
                  return (
                    <OutgoingChat
                      key={i}
